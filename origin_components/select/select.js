...
document.querySelectorAll("[data-select]").forEach((select) => {
  ...
  const selectInput = select.querySelector("[data-select__input]");
  const selectOptions = select.querySelector("[data-select__options]");
  ...
  select.addEventListener("keydown", (event) => {
    ...
    if (selectOptions.classList.contains("select__options_visible")) {
      if (
        event.key === "ArrowDown" &&
        selectedOption !== selectOptions.lastElementChild
      ) {
        selectedOption.removeAttribute("data-selected");
      ...
      if (
        event.key === "ArrowUp" &&
        selectedOption !== selectOptions.firstElementChild
      ) {
        selectedOption.removeAttribute("data-selected");
