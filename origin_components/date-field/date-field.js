document.querySelectorAll("[data-date-field]").forEach(dateField => {
  ...
  datePickerDates = datePicker.querySelector("[data-date-picker__dates]");
  ...
  const generateCalendar = () => {
    ...
    let
    monthFirstWeekDay = weekDays.get(new Date(year, month, 1).getDay()), /* Какой день недели первое число месяца */
    daysInMonth = new Date(year, month+1, 0).getDate(), /* Количество дней в месяце */
    monthLastWeakDay = weekDays.get(new Date(year, month, daysInMonth).getDay()), /* Какой день недели последнее число месяца */
    daysInLastMonth = new Date(year, month, 0).getDate(), /* Количество дней в прошлом месяце */
    dates = "";
    ...
    datePickerDates.querySelectorAll("[data-date-picker__date-cell]").forEach(cell => {
